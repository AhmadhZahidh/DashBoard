...
// ── Helpers ──────────────────────────────────────────────────
const GAMING_AVATARS = ['G','F','Z','D','T','A','R','P','B','X','C','S','K','M','N','L'];
const COLORS = ['#7c3aed','#10b981','#3b82f6','#f59e0b','#ec4899','#06b6d4','#8b5cf6','#14b8a6'];
const EMOJIS = ['😀','😂','🔥','💯','👍','❤️','🎮','⚡','💎','🏆','😎','🤩','💪','🙏','👀','🎯','🚀','💀','😈','🤖','✨','🌟','💫','🎲','��','🤑','😍','🥳','🎉','🔑'];

const getAvatar = (u) => GAMING_AVATARS[(u?.charCodeAt(0)||0) % GAMING_AVATARS.length] || 'G';
const getColor  = (u) => COLORS[(u?.charCodeAt(0)||0) % COLORS.length] || '#7c3aed';
...
export default function LiveChat() {
  ...
  const userId          = useRef(user?._id);
  const spamRef         = useRef({ count: 0, lastTime: 0 });

  // Update userId ref when user changes
  useEffect(() => { userId.current = user?._id; }, [user]);
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        await setOnline(user._id, user.username, user.role, getAvatar(user.username));
        setConnected(true);
      } catch (e) {
        setConnected(true); // Still show as connected even if presence fails
      }

      // ── Messages listener ──────────────────────────────────
      unsubMsgs = listenGlobalMessages((msgs) => {
        setMessages(msgs);

        // Play sound for new messages from others
        if (msgs.length > lastMsgCount.current) {
          const newest = msgs[msgs.length - 1];
          if (newest && newest.userId !== userId.current && !newest._optimistic) {
            playNotif();
          }
        }
      ...
      unsubOnline = listenOnline(setOnlineUsers);

      // ── Typing listener ────────────────────────────────────
      unsubTyping = listenTyping(user._id, setTypers);
    };
  ...
  const handleSend = useCallback(async (e) => {
    ...
    if (!msg || !user?._id) return;
    if (msg.length > 500) { toast.error('Message too long (max 500 chars)'); return; }

    // Spam protection
    const now = Date.now();
    const spam = spamRef.current;
    if (now - spam.lastTime < 1000) {
      spam.count++;
      if (spam.count >= 5) { toast.error('Slow down! Too many messages.'); return; }
    } else {
      spam.count = 0;
    }
    spam.lastTime = now;
    ...
    const optimisticId = 'opt_' + now + '_' + Math.random().toString(36).substr(2, 5);
    const optimisticMsg = {
      id: optimisticId,
      userId: user._id,
    ...
    setMessages(prev => {
      // Remove any previous optimistic messages and add new one
  ...
  }, [input, user?._id, user?.username, user?.role, scrollToBottom]);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <p style={{ color: '#505070', fontSize: 12, marginTop: 2 }}>
            {connected ? ('Firebase Realtime - ' + onlineUsers.length + ' online') : 'Connecting to Firebase...'}
          </p>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'rgba(14,14,31,0.95)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 18, overflow: 'hidden', backdropFilter: 'blur(20px)', minWidth: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {messages.map((msg, i) => {
              ...
              const msgDate = fmtDate(msg.timestamp || msg.clientTime);
              ...
              return (
                <React.Fragment key={msg.id || i}>
                  ...
                  <div style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 8, marginBottom: 6, animation: 'fadeInUp 0.25s ease', opacity: msg._optimistic ? 0.75 : 1 }}>
                    {!mine && (
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: color + '20', border: '2px solid ' + color + '50', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0, fontWeight: 700, color: color }}>
                    ...
                    <div style={{ maxWidth: '72%', minWidth: 60 }}>
                      ...
                      <div style={{ position: 'relative' }} className="bubble-wrap">
                        ...
                        {user?.role === 'admin' && !msg.deleted && !msg._optimistic && (
                          <button onClick={() => handleDelete(msg.id)} className="delete-btn" style={{ position: 'absolute', top: -8, right: mine ? 'auto' : -8, left: mine ? -8 : 'auto', background: 'rgba(239,68,68,0.9)', border: 'none', color: '#fff', width: 20, height: 20, borderRadius: '50%', cursor: 'pointer', fontSize: 10, display: 'none', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                        )}
                      </div>
                      <div style={{ fontSize: 10, color: '#505070', marginTop: 3, textAlign: mine ? 'right' : 'left' }}>
                        {msg._optimistic ? 'Sending...' : fmtTime(msg.timestamp || msg.clientTime)}
                        {mine && !msg._optimistic && ' ✓'}
                      </div>
                    </div>
                    {mine && (
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#7c3aed,#10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0, fontWeight: 700, color: '#fff' }}>
            ...
            <form onSubmit={handleSend} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                <input
                  ...
                  maxLength={500}
                  disabled={!connected}
                  style={{ width: '100%', background: 'rgba(14,14,31,0.9)', border: '1px solid rgba(124,58,237,0.25)', color: '#fff', padding: '11px 16px', borderRadius: 12, fontSize: 14, fontFamily: 'Space Grotesk,sans-serif', outline: 'none', transition: 'border-color 0.2s' }}
                  onFocus={e => e.target.style.borderColor = 'rgba(124,58,237,0.6)'}
                  onBlur={e => e.target.style.borderColor = 'rgba(124,58,237,0.25)'}
                />
                {input.length > 400 && (
                  <span style={{ position: 'absolute', right: 10, bottom: -18, fontSize: 10, color: input.length > 480 ? '#ef4444' : '#505070' }}>{input.length}/500</span>
                )}
              </div>
              <button type="submit" disabled={sending || !input.trim() || !connected} style={{ background: (sending || !input.trim() || !connected) ? 'rgba(124,58,237,0.3)' : 'linear-gradient(135deg,#7c3aed,#5b21b6)', border: 'none', color: '#fff', width: 46, height: 42, borderRadius: 12, cursor: (sending || !input.trim() || !connected) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0, transition: 'all 0.2s', boxShadow: (sending || !input.trim()) ? 'none' : '0 4px 15px rgba(124,58,237,0.4)' }}>
                {sending
          ...
          <div style={{ width: 220, background: 'rgba(14,14,31,0.95)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 18, display: 'flex', flexDirection: 'column', overflow: 'hidden', backdropFilter: 'blur(20px)', flexShrink: 0, animation: 'slideInLeft 0.3s ease' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(124,58,237,0.12)', background: 'rgba(16,185,129,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 13, color: '#10b981', fontFamily: 'Space Grotesk,sans-serif' }}>Online ({onlineUsers.length})</span>
              </div>
            </div>
                    ...
                    return (
                      <div key={u.id || i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 10, marginBottom: 4, background: isMe ? 'rgba(124,58,237,0.12)' : 'transparent', border: isMe ? '1px solid rgba(124,58,237,0.2)' : '1px solid transparent', transition: 'all 0.2s' }}>
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                          <div style={{ width: 34, height: 34, borderRadius: '50%', background: color + '20', border: '2px solid ' + color + '50', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: color }}>
            ...
            <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(124,58,237,0.12)', background: 'rgba(124,58,237,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#7c3aed,#10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#fff' }}>
                  {getAvatar(user?.username)}
                </div>
                <div>
